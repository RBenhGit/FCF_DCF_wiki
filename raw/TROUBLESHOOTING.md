...
### **P/B Valuation Issues**

#### **Issue: "Unable to calculate book value per share" (Excel mode)**

```
❌ P/B Analysis Error: Unable to calculate book value per share
```

This error occurs in pure Excel mode even when all three financial statement files are present.

**Root causes (all fixed as of 2026-05-21):**

1. `_extract_shareholders_equity()` was reading the wrong DataFrame column (index 0 instead of the actual label column, which is index 2 in the standard Excel template).
2. The label `'Other Common Equity Adj'` was matched before `'Common Equity'` because `'common equity'` is a substring of it.
3. `shares_outstanding` was never populated from Excel — it was only set via a yfinance call that does not happen in offline/Excel-only mode.

**Solution:** All three bugs are fixed. `_extract_shares_from_income_statement()` now reads "Weighted Average Diluted Shares Out." from the income statement DataFrame before falling back to any network call. See [`docs/PB_EXCEL_BVPS_FIX.md`](PB_EXCEL_BVPS_FIX.md) for the full analysis.

**If the error persists after the fix:**
- Verify the Income Statement file contains a "Weighted Average Diluted Shares Out." row.
- Verify the Balance Sheet contains a "Common Equity" or "Total Equity" row with numeric values.
- Check the log output for `"Extracted shares from income statement"` and `"Found shareholders' equity from DataFrame row"` messages to confirm both values were found.
